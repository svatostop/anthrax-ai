...
{
    if (first_run) {
        prev_swapchain_index = swapchain_size - 1;
        first_run = false;
    }
    else {
        prev_swapchain_index = swapchain_index;
    }
	VkResult e = vkAcquireNextImageKHR(dev, swapchain, 1000000000, present_sema[prev_swapchain_index], VK_NULL_HANDLE, &swapchain_index);
...
{		
    ...
    present_sema.resize(swapchain_size);
